...
//        val azimuth = if(abs(_azimuth) == 0.0.toFloat()){
//            onCoinTap()
//        }else{
//            _azimuth
//        }
        ...
        Log.d("_Pitch:" , "$_pitch")
        Log.d("Pitch:","${abs(pitch)}" )

        if(abs(pitch)> 1.3962625 && abs(pitch) < 1.5707964)
    ...
    private fun onCoinTap(){
        ...
        if (randomNumber == 1){
            flipTheCoin(R.drawable.head_coin, "Heads")
        } else {
            flipTheCoin(R.drawable.tail_coin, "Tails")
        }
    }
}